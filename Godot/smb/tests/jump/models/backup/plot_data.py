...
for dossier in dossiers:
    chemin_fichier = os.path.join(dossier, 'data.csv')
...
plt.xlabel('Step')
plt.ylabel('Reward')
plt.title('IL + RL VS RL')

# Ajouter une grille pour améliorer la lisibilité
plt.grid(True, linestyle='--', alpha=0.6)

# Afficher la légende
plt.legend(title='Expériences', loc='upper right', fontsize='small')

# Réduire les marges
plt.tight_layout(pad=1.0)

# Sauvegarder le graphique avec des marges réduites
plt.savefig('comparaison_graphique_publication.png', dpi=300, bbox_inches='tight')
